...
function AuthorComponent({ author, timestamp }) {
  return (
    <div className='Author'>
      {author !== undefined && author.content !== undefined ?
        <>
          <Avatar
            sx={{ border: '1px solid #cccccc' }}
            className='authorImage'
            src={author.content !== undefined ? `${config.api.protocol}://${config.api.host}/images/${author.node_id}/profile/${author.content.profileImageName}` : ''} />
          <div>
